...
def get_file_ids(section1,section2):
    with open('organized_files.yaml', 'r') as file: 
...
def get_files(file_type : str, parent_folder_list : list[str], num_documents : int) -> list[File]:
    """"Loads the file ids from the organized_files.yaml file and returns a list of files."""
    with open('asdf.yaml', 'r') as file: 
        file_ids = yaml.safe_load(file)
...
def get_all_files(num_documents: int) -> list[File]:

    parent_folder_list = [str(year) for year in range(2014, 2025)]

    word_files = get_files(file_type='word',parent_folder_list=parent_folder_list,num_documents=num_documents)
    print(f"found {len(word_files)} word files")
    for file in word_files:
        print(file.file_name)
    pdf_files = get_files(file_type='pdf',parent_folder_list=parent_folder_list,num_documents=num_documents)
